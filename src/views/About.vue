<template>
  <div class="flex-col items-center">
   <!-- <h1 class="text-gray-700 text-6xl">API detección de animales</h1>
    <p class="text-gray-600 text-xl text-center">
      Proyecto para obtener la clasificación de imagenes de animales 
      <br />Este proyecto utiliza: vue js, 
    </p>

    <router-link
      to="/"
      class="mt-5 text-xl text-green-600 hover:underline"
    >Volver a la pagina de Inicio</router-link>-->
  </div>
  
 

  

  

<div><px-nosotros /></div>

  
</template>

<script>

import PxNosotros from "@/components/PxNosotros";

export default {
  name: "Nosotros",
  components: {PxNosotros }
};
</script>